...
def to_gemini_format(history):
    ...
    for m in history:
        ...
        # 🚀 关键修复：parts 内部必须是字典 {"text": "内容"}，不能直接是字符串
...
# 1. 获取用户信息（确保数据库返回了 nickname 和 glassware 字段）
    ...
    system_instruction = f"""
        ...
        """    
    # 🚀 关键修复：在这里用刚刚生成的 instruction 初始化模型
# 定义生成配置（包含你的 MW 身份注入和工具挂载）
    chat_config = types.GenerateContentConfig(
        system_instruction=system_instruction, # 注入你的 MW/MS 灵魂
        tools=[google_search],                 # 挂载搜索工具
        automatic_function_calling=types.AutomaticFunctionCallingConfig(disable=False) # 开启自动调用
    )
    def generate():
        full_reply = ""
        try:
            ...
            chat_session = client.chats.create(
                model='gemini-3.1-pro-preview', # 或者最新的 gemini-3-flash
                config=chat_config,
                history=to_gemini_format(cellar_db.get_recent_history(user_id, limit=8))
            )
            
            # 开启极速流式响应
            response = chat_session.send_message_stream(message=user_query)
            ...
            for chunk in response:
                for part in chunk.candidates[0].content.parts:
                    ...
                    if part.function_call:
                        ...
                        # 提取 Gemini 生成的关键词
                        # 这里的 'query' 必须对应你 google_search 函数定义的参数名
                        search_keyword = part.function_call.args.get('query', '')
                        is_english = all(ord(char) < 128 for char in user_query[:10]) 
                        search_msg = f"🔍 CellarEcho 正在执行深度搜索: {search_keyword}" if not is_english else f"🔍 CellarEcho is performing deep search: {search_keyword}"
                        yield f"{search_msg}\n\n" 
...
def run_summary_in_background(user_id, long_term_summary, old_text):
    ...
    try:
        ...
        res = client.models.generate_content(
            model='gemini-3.1-pro-preview', # 绝对锁定最新模型
