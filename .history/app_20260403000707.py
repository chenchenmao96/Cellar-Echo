...
def run_summary_in_background(user_id, long_term_summary, old_text):
    ...
    try:
        ...
        - 酒杯资产：{curr_glassware}
        - 已知事实：{curr_facts}
        - 长期偏好摘要：{long_term_summary}
        
        【任务说明】：
        1. 检查称呼变更（如“叫我陈陈”）。
        2. 检查酒杯变动（如新买 Zalto, Grassl）。
        3. 提取【硬事实】：包括但不限于居住地、家人(如Eric)、职业(HCI博士)、食物禁忌、专业水平(WSET 3)等。
        4. 总结【近期动态】：将这12条对话与旧摘要融合，写出一段 200 字以内的综合摘要。
        
        请严格按 JSON 返回：
        {{
            "nickname": "最新昵称或保持原样",
            "glassware": "最新酒杯列表字符串",
            "facts": {{"key": "value"}},  // 仅包含本次新发现或需要更新的事实
            "summary": "更新后的叙事性摘要"
        }}
        """
