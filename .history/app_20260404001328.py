...
def perform_google_search(query, user_facts=None):
    ...
    try:
        ...
        print("SERPER raw text:", response.text[:1000])

        if response.status_code != 200:
            return f"搜索组件返回异常状态码: {response.status_code}"
        ...
        # fallback: answerBox / knowledgeGraph
        answer_box = results.get("answerBox")
        if answer_box:
            return f"answerBox: {json.dumps(answer_box, ensure_ascii=False)}"

        knowledge_graph = results.get("knowledgeGraph")
        if knowledge_graph:
            return f"knowledgeGraph: {json.dumps(knowledge_graph, ensure_ascii=False)}"

        return f"未找到相关实时互联网信息。query={query}"
        ...
        try:
            ...
            if ai_message.tool_calls:
                ...
                for tool_call in ai_message.tool_calls:
                    print("RAW TOOL CALL:", tool_call)
                    print("RAW ARGUMENTS:", getattr(tool_call.function, "arguments", None))

                    raw_args = getattr(tool_call.function, "arguments", None)

                    if not raw_args:
                        print("No arguments found in tool call")
                        continue
                    ...
                    try:
                        ...
                        print("Bad arguments:", raw_args)
                        continue

                    search_query = args.get("query")
                    print("PARSED QUERY:", search_query)

                    if not search_query:
                        print("No query field found in parsed arguments:", args)
                        continue
                combined_search_text = "\n\n".join(search_results_combined)
    # 第二轮：不要复用原 messages
    # 重新组一个“只允许直接回答”的上下文
                second_pass_messages = [
                    {
                        "role": "system",
                        "content": (
                            f"You are CellarEcho, a professional MW/MS-level wine assistant for {nickname}. "
                            "The external search has already been completed. "
                            "Your task now is ONLY to answer the user's original question clearly and directly. "
                            "Do not search again. "
                            "Do not plan any search. "
                            "Do not mention tools, WINE_TOOLS, google_search, DSML, function_calls, invoke, or parameter. "
                            "Do not output any internal reasoning or markup. "
                    ...
                    {
                        "role": "user",
                        "content": "请基于上面的信息，直接给出最终答案。不要再次搜索，不要输出任何工具调用痕迹。"
                    }
                ]
