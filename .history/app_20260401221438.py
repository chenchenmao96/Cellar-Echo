...
def fetch_ct_data():
    ...
    # 1. 确保账号密码经过了 URL 编码
    safe_user = urllib.parse.quote(CT_USER) if CT_USER else ""
    safe_pass = urllib.parse.quote(CT_PASS) if CT_PASS else ""
    
    url = f"https://www.cellartracker.com/xlquery.asp?User={safe_user}&Password={safe_pass}&Format=csv&Table=List"
