...
WINE_TOOLS = [
    {
        ...
        "function": {
            ...
            "description": """
            【全维度上下文感知搜索协议】：
            你必须作为一个拥有‘深层共情’能力的顶级管家，在发起搜索前完成以下推理：
            
            1. **多维特征提取 (Aspect-Agnostic Extraction)**：
               深入分析【客户档案】中的所有历史碎片。不要局限于特定类别，而是通过用户的语言风格、重复提及的话题、以及未表达的潜在需求，推断其此刻的‘好奇心矢量’。
               （例如：用户近期是否在关注气候变化对产区的影响？是否在寻找适合赠予特定长辈的礼物逻辑？是否关注酒庄的女性酿酒师背景？）
            
            2. **时空与状态同步 (Spatio-Temporal Sync)**：
               自动结合【当前系统时间】和【用户实时坐标】。如果用户在旅行中，搜索词应自动切换至当前目的地的零售和适饮场景。
            
            3. **语义查询扩充 (Semantic Query Expansion)**：
               将用户模糊的输入（如“这瓶怎么样”）重构成一个极其专业的、包含所有【已知背景】和【推断关注点】的搜索长句。
            """,
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "由 AI 实时合成的、全维度的、包含所有隐性上下文的搜索指令"}
                },
                "required": ["query"]
            }
        }
