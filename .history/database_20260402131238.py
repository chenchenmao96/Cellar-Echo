...
class CellarDB:
    ...
    # --- 2. 更新长期摘要 ---
    def update_user_summary(self, user_id, new_summary):
        """覆盖更新长期偏好"""
        self.profile_collection.update_one(
            {"user_id": user_id},
            {"$set": {"summary": new_summary, "last_updated": time.time()}},
            upsert=True
        )
    ...
    def get_recent_history(self, user_id, limit=8):
        ...
        return [{"role": h["role"], "content": h["content"]} for h in list(cursor)[::-1]]
