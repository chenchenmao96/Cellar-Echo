            ...
            for chunk in response:
                            ...
                            for part in candidate.content.parts:
                                if part.function_call:
                                    ...
                                    # 兼容不同可能的参数名
                                    query = fn_call.args.get('query') or fn_call.args.get('search_query') or "深度分析"
                                    is_zh = not all(ord(char) < 128 for char in user_query[:10])
                                    msg = f"\n🔍 CellarEcho 正在联网查询: {query}...\n\n" if is_zh else f"\n🔍 CellarEcho is searching: {query}...\n\n"
                                    yield msg

                                # 🚀 逻辑 B：安全提取文本，避开 .text 属性的报错
                                if part.text:
                                    full_reply += part.text
                                    yield part.text
                            
                            # 🛡️ 逻辑 C：处理安全拦截（酒类价格常被误拦）
                            if candidate.finish_reason == 'SAFETY':
                                yield "\n⚠️ [部分内容因安全策略被拦截，可能涉及受限信息]\n"
            # 3. 结束后保存到数据库
            if full_reply:
