...
# 1. 获取用户信息（确保数据库返回了 nickname 和 glassware 字段）
    ...
    # 2. 获取库存并检查状态
    inventory = cellar_db.get_inventory_for_ai(user_id, limit=1000)
    ...
    if not nickname:
        ...
        if is_inventory_loading:
                    # 只有当数据库真的没数据时才显示这个
            prompt_content = f"你是 'CellarEcho'。请礼貌告知 {nickname}，你还没看到他的酒单，提醒他系统正在自动同步，请稍等。"
        else:
            # 👈 强化指令：不准 AI 说“我看不到”
            prompt_content = f"""
            ...
            【已知偏好】：{long_term_summary}
            【使用酒杯】：{glassware}
            
            【要求】：当用户询问酒柜内容时，你必须直接查阅上方清单并给出 WSET 3 级 水准的建议。
            严禁回复“我无法查看您的酒柜”。
            回复语言：{lang}。
            """
