...
# 1. 获取用户信息（确保数据库返回了 nickname 和 glassware 字段）
    ...
    def generate_stream():
        full_response = ""
        search_triggered = False # 状态锁，防止重复发送信号
        try:
        ...
        # 使用流式接口
            ...
            for chunk in responses:
                ...
                if not search_triggered and metadata and metadata.search_queries:
                    # 只要探测到有搜索词，立刻发出信号，不等待文字
                    print(f"\n[🔍 发现搜索意图: {metadata.search_queries}]")
                    yield "__SEARCHING_START__" 
