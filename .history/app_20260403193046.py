    ...
    def generate():
        ...
        try:
            ...
            if ai_message.tool_calls:
                ...
                # 🚀 【核心修复 A】：给第二次呼叫打一针“清醒剂”
                # 强迫 AI 意识到搜索已完成，直接给结论，禁止输出任何标签
                messages.append({
                    "role": "system", 
                    "content": "CRITICAL: The search is complete. Now, output ONLY the professional MW analysis. DO NOT repeat the tool call or use any <|DSML|> tags."
                })
            ...
            for chunk in response:
                if chunk.choices[0].delta.content:
                    ...
                    clean_content = re.sub(r'<[^>]*DSML[^>]*>|<[^>]*function_calls[^>]*>|<[^>]*invoke[^>]*>', '', content, flags=re.IGNORECASE)
                    
                    # 针对被流式切碎的单个符号进行二次拦截
                    if clean_content.strip() in ["<", "|", "DSML", "function_calls", ">", "invoke"]:
                        continue
                    full_reply += clean_content
                    yield clean_content # 👈 实时推送到前端
