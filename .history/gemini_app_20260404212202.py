...
# 1. 获取用户信息（确保数据库返回了 nickname 和 glassware 字段）
    ...
    def generate_stream(user_query, config):
        # 使用流式接口
        ...
        for chunk in responses:
            if chunk.text:
                ...
                # SSE 格式：必须以 "data: " 开头并以 "\n\n" 结尾
                yield f"data: {json.dumps({'text': chunk.text})}\n\n"
        
        # 结束后可以将完整回复存入数据库
        # 注意：这里需要通过某种方式获取 user_id
        # cellar_db.save_chat_and_check_limit(user_id, "model", "".join(full_response))
        # 8. 返回流式响应
    return Response(generate_stream(user_query, config), mimetype='text/plain', headers={
