    ...
    def generate():
        ...
        try:
            ...
            for chunk in response:
                # 🚀 方案 A：安全获取最终文字（最推荐，最稳）
                if chunk.text:
                    full_reply += chunk.text
                    yield chunk.text
                
                # 🚀 方案 B：如果你非要显示“正在搜索”的状态，请用 hasattr 检查
                # 2026 SDK 中，自动函数调用的中间状态可以通过这种方式捕获
                if hasattr(chunk, 'tool_calls') and chunk.tool_calls:
                    for call in chunk.tool_calls:
                        keyword = call.args.get('query', '')
                        is_zh = not all(ord(char) < 128 for char in user_query[:10])
                        msg = f"\n🔍 CellarEcho 正在搜索: {keyword}\n\n" if is_zh else f"\n🔍 CellarEcho is searching: {keyword}\n\n"
                        yield msg
