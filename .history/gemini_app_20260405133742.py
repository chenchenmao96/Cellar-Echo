...
# 1. 获取用户信息（确保数据库返回了 nickname 和 glassware 字段）
    ...
    def generate_stream():
        full_response = ""
        search_signal_sent = False
        try:
            # 🚀 模拟动态思考：先吐出几个点，每 0.5 秒一个
            # 这会让前端立刻开始渲染，用户会看到点点点在跳动
            yield " "
        # 使用流式接口
            ...
            for chunk in responses:
                if hasattr(chunk.candidates[0], "grounding_metadata"):
                    ...
                    if metadata:
                        ...
                        search_signal_sent = True
                if chunk.text:
                    text_fragment = chunk.text
                    full_response += text_fragment
                    yield text_fragment
                if search_signal_sent:
                    attribution = "\n\n> *注：此建议由 Google 搜索提供实时数据支持。*" if lang == "zh" else "\n\n> *Note: This response includes information from Google Search.*"
                    full_response += attribution
                    yield attribution # 这里的文字会像普通对话一样流出来
