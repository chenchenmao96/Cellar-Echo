    ...
    def generate():
        ...
        try:
            ...
            if ai_message.tool_calls:
                ...
                clean_message = [
                    messages[0],   # MW/MS 专家人格与酒柜数据
                    messages[-1],  # 用户的原始提问
                    ai_message     # AI 发起的 tool_calls 请求
                ]
                ...
                final_messages = [
                                    messages[0], # 找回灵魂：你的 MW/MS 身份和酒柜数据
                                ]
                                
                # 找回记忆：带上之前的对话滑窗，防止它变“傻”
                final_messages.extend(recent_history)
                
                # 注入事实：把搜到的拉菲价格/评分作为“背景资料”直接喂给它
                final_messages.append({"role": "user", "content": user_query})
                final_messages.append({
                    "role": "system", 
                    "content": f"REAL-TIME DATA SECURED:\n{chr(10).join(search_results_combined)}"
                })
                final_messages.append({
                    "role": "system", 
                    "content": "请基于以上最新数据直接给出最终分析。严禁再次搜索，严禁输出任何 <|DSML|> 标签。"
                })                
