...
def login():
    data = request.json
    ct_user = data.get("username")
    ct_pass = data.get("password")
    
    # 尝试连接 CT 进行验证
    test_url = f"https://www.cellartracker.com/xlquery.asp?User={ct_user}&Password={ct_pass}&Format=csv&Table=Inventory"
    
    try:
        response = requests.get(test_url, timeout=10)
        # 验证逻辑：如果返回的内容包含 "Wine" 这个表头，说明登录成功
        if response.status_code == 200 and "Wine" in response.text:
            # 登录成功，我们可以把密码暂时存入 session（或者让前端传回来）
            return jsonify({
                "status": "success", 
                "user_id": ct_user, # 以后都用这个当 ID
                "message": "登录成功"
            })
        else:
            return jsonify({"status": "error", "message": "CT 的账号密码不对"}), 401
    except Exception as e:
        return jsonify({"status": "error", "message": f"连接 CT 失败: {str(e)}"}), 500
# 【新增】处理前端同步请求的路由
