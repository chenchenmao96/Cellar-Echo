...
# 1. 获取用户信息（确保数据库返回了 nickname 和 glassware 字段）
    ...
    def generate():
            ...
            try:
                ...
                # 🚀 致命修复：SDK 原生支持安全提取，别再去扒底层结构了！
                for chunk in response:
                    
                    # A. 抓取文本（chunk.text 内部自带防崩溃机制，遇空则跳过）
                    if chunk.text:
                        full_reply += chunk.text
                        yield chunk.text
                    
                    # B. 抓取工具调用状态（原生支持，直接读取）
                    # B. 暴力捕获工具调用：只要 chunk 带有属性，就尝试提取
                    elif hasattr(chunk, 'function_calls') and chunk.function_calls:
                        for fc in chunk.function_calls:
                            query = fc.args.get('query', '相关信息')
                            yield f"\n🔍 CellarEcho 正在联网搜寻圣路易斯本地情报: {query}...\n\n"
